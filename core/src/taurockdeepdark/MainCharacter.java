...
public class MainCharacter implements ApplicationListener {
    ...
    public String getTileID(float fX, float fY, int nWidth) {// this is slightly complicated but its basically grabbing the tile that the character is standing on and getting the ID
        boolean bCollided = false;
        String sID;
        Object obj;
        sID = armMaps[nCurrentMap].arclCollisionLayer[nLayerCount].getCell((int) ((fX + nWidth / 4) / tileWidth), (int) (fY / tileHeight))
                .getTile().getProperties().get("BlockType").toString();
    ...
    public void render() {
        ...
        fCharacterY += fCharacterVelocityY;
        sID = getTileID(fCharacterX, fCharacterY, nCharacterWidth);//Did it touched a tile with the block ID

        if (sID.equals("Wall")) {//If it touched a tile with the block ID reset the position
            fCharacterX = fOldX;
            fCharacterY = fOldY;
        }

        if (sID.equals("MoveUp")) {//This checks if the character is standing on a door
        ...
        if (sID.equals("Ground")) {//If the character has moved off the door the map can be changed again
            bJustSet = false;
        }


        if (sID.equals( "MoveDown")) {//Same as the previous bit but for the door the decreases the map index
